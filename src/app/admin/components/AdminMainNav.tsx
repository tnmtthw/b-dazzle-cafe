...
import { useSidebar } from "@/data/sidebar";
...
const AdminMainNav = ({ children }: AdminNavbarProps) => {
  ...
  const sidebarItems = [
    ...
    { icon: <Coffee size={20} />, name: 'Products', href: '/admin/productcontrol', badge: data?.products },
  ...
  return (
    <div className="flex h-screen bg-gray-100">
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {sidebarItems.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className={`flex items-center ${collapsed && !isMobile ? 'justify-center' : 'justify-between'} px-3 py-3 rounded-lg transition-colors ${pathname === item.href || (item.href !== '/admin' && pathname?.startsWith(item.href))
      ...
      {/* Main Content Area */}
      <div className={`flex-1 ${isMobile ? 'ml-0' : (collapsed ? 'ml-20' : 'ml-64')
